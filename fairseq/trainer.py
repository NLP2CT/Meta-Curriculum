...
class Trainer(object):
    ...
    def init_meters(self, args):
        ...
        self.meters['forward_loss'] = AverageMeter()
        self.meters['valid_nll_loss'] = AverageMeter()
        self.meters['forward_nll_loss'] = AverageMeter()
...
#                print('| NOTICE: your device may support faster training with --fp16')
    ...
    def forward_step(self, sample, raise_oom=False):
        """Do forward pass in evaluation mode, but keep track of computation graph"""
        self.model.eval()
        self.criterion.eval()
        ...
        try:
            loss, sample_size, logging_output = self.task.forward_step(
                sample, self.model, self.criterion, self.optimizer
            )
        except RuntimeError as e:
            if 'out of memory' in str(e) and not raise_oom:
                ...
                if self.cuda:
                    torch.cuda.empty_cache()
                return self.forward_step(sample, raise_oom=True)
            else:
                raise e
        ...
        # update meters for forward
        ntokens = logging_output.get('ntokens', 0)
        self.meters['forward_loss'].update(logging_output.get('loss', 0), sample_size)
        if 'forward_acc' in self.meters:
            self.meters['forward_acc'].update(
                logging_output.get('acc', 0), sample_size)

        if 'nll_loss' in logging_output:
            self.meters['forward_nll_loss'].update(logging_output.get('nll_loss', 0), ntokens)
