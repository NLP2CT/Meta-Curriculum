...
class IndexedRawLinesDataset(torch.utils.data.Dataset):
    """Takes a list of lines as input and binarizes it in memory at instantiation.
    Original lines are also kept in memory"""
    ...
    def __init__(self, lines, dictionary, append_eos=True, reverse_order=False):
        ...
        self.append_eos = append_eos
        self.reverse_order = reverse_order
        self.read_data(lines, dictionary)
        self.size = len(self.tokens_list)

    def read_data(self, lines, dictionary):
        for line in lines:
            self.lines.append(line.strip('\n'))
            tokens = dictionary.encode_line(
                line, add_if_not_exist=False,
