...
def get_clustering_parser(default_task='translation'):
    parser = get_parser('clustering', default_task)
    add_dataset_args(parser, train=True)
    add_distributed_training_args(parser)
    add_model_args(parser)
    add_checkpoint_args(parser)
    add_generation_args(parser)
    parser.add_argument('--src-encoding-file', type=str)
    parser.add_argument('--tgt-encoding-file', type=str)
    parser.add_argument('--readable-clusters-file', type=str)
    parser.add_argument('--split-dir', type=str)
    parser.add_argument('--K', '-K', type=int)
    parser.add_argument('--max-points-per-centroid', type=int)
    parser.add_argument('--max-K', type=int)
    return parser


def get_meta_learning_parser(default_task='translation'):
    parser = get_parser('Meta Learner', default_task)
    dataset_group = add_unrestricted_dataset_args(parser, train=True)
    # Also add --test-subset
    dataset_group.add_argument('--test-subset', default='test', metavar='SPLIT',
                               help='data subset to use for testing (train, valid, test)')
    add_distributed_training_args(parser)
    add_optimization_args(parser)
    add_checkpoint_args(parser)
#    generation_group = add_generation_args(parser)
#    generation_group.add_argument('--lowercase', action='store_true', help='lowercase data for sacrebleu evaluation')
    parser.add_argument('--criterion', default='cross_entropy', metavar='CRIT', choices=CRITERION_REGISTRY.keys(), help='Training Criterion')
    return parser

def get_meta_finetune_parser(default_task='translation'):
    parser = get_parser('Meta Learner', default_task)
    dataset_group = add_unrestricted_dataset_args(parser, train=True)
    # Also add --test-subset
    dataset_group.add_argument('--test-subset', default='test', metavar='SPLIT',
...
# TODO can we merge this with get_meta_learning_parser?
def get_meta_training_parser(default_task='meta-curriculum'):
    parser = get_parser('Meta Learner', default_task)
    # Also add --test-subset
    parser.add_argument('--test-subset', default='test', metavar='SPLIT',
                        help='data subset to use for testing (train, valid, test)')
    parser.add_argument('--baseline', action='store_true', help='whether to compute baseline')
    add_unrestricted_dataset_args(parser, train=True)
    add_distributed_training_args(parser)
    add_optimization_args(parser)
    add_checkpoint_args(parser)
    parser.add_argument('--train-percentage', type=float, default=0.8, help='percentage of tasks to use for training')
    parser.add_argument('--dev-percentage', type=float, default=1.0, help='percentage of dev/test tasks to use for dev')
    parser.add_argument('--domains', nargs='*', help='List of domains to consider')
...
Args = namedtuple('Args', ['task',  'criterion', 'train_subset', 'max_tokens', 'cpu', 'fp16', 'max_sentences',
                           'required_batch_size_multiple', 'seed',  'distributed_world_size', 'distributed_rank',
                           'num_workers', 'max_epoch', 'max_update', 'optimizer', 'lr', 'momentum', 'weight_decay',
                           'lr_scheduler', 'warmup_init_lr', 'warmup_updates', 'min_lr', 'update_freq',
                           'fix_batches_to_gpus', 'curriculum', 'log_format', 'tensorboard_logdir',
                           'valid_subset', 'clip_norm', 'save_interval_updates', 'validate_interval',
                           'max_sentences_valid', 'skip_invalid_size_inputs_valid_test', 'save_interval',
                           'no_save', 'no_epoch_checkpoints', 'save_dir', 'keep_last_epochs'])


def get_meta_learning_args():
    # TODO support distribuited
    # TODO review fix batches to gpu
    return Args(task='meta', criterion='reptile', train_subset='train', max_tokens=1, cpu=False,
                fp16=False, max_sentences=None, required_batch_size_multiple=8, seed=1, distributed_world_size=1,
                distributed_rank=0, num_workers=0, max_epoch=0, max_update=0, optimizer='sgd', lr=[0.02],
                momentum=0.0, weight_decay=0.0, lr_scheduler='immutable_inverse_sqrt', warmup_init_lr=-1,
                warmup_updates=4000, min_lr=-1, update_freq=[1], fix_batches_to_gpus=False, curriculum=0,
                log_format='none', tensorboard_logdir='', valid_subset='valid', clip_norm=25,
                save_interval_updates=1000, validate_interval=1, max_sentences_valid=None,
                skip_invalid_size_inputs_valid_test=True, save_interval=100000, no_save=True, no_epoch_checkpoints=True,
                save_dir='meta_checkpoints', keep_last_epochs=-1)
...
def get_parser(desc, default_task='translation'):
    ...
    parser.add_argument('--flush-secs', default=1, type=int, help='flush_secs for tensorboard')
    parser.add_argument('--max-queue', default=10, type=int, help='max_queue size for tensorboard')
...
def add_unrestricted_dataset_args(parser, train=False, gen=False):
